...
import {
  ...
  ListChecks, LogOut, Menu, PackageCheck, PackageSearch, Receipt, Route as RouteIcon, Settings,
...
import { GlobalSearch } from "@/components/farazz/global-search";
...
const GROUPS: Group[] = [
  { titleKey: "nav.overview", items: [{ to: "/", labelKey: "nav.dashboard", icon: Home }] },
  {
    ...
    items: [
      ...
      { to: "/tracking", labelKey: "nav.tracking", icon: PackageSearch },
      { to: "/shipments-create", labelKey: "nav.createShipment", icon: PackageCheck },
...
function NotificationsPopover() {
  ...
  // Derive a handful of recent, high-signal notifications from live data.
  const notifications = (() => {
    const items: { icon: ReactNode; title: string; sub: string; to: string; tone: string }[] = [];
    db.shipments
      .filter((s) => s.status === "exception" || s.status === "at_risk")
      .slice(0, 2)
      .forEach((s) =>
        items.push({
          icon: <CircleAlert className="h-4 w-4 text-critical" aria-hidden />,
          title: `Exception on ${s.tracking}`,
          sub: `${s.origin} → ${s.destination} · ${s.currentLocation}`,
          to: "/exceptions",
          tone: "critical",
        }),
      );
    db.approvals
      ...
      .forEach((a) =>
        items.push({
          icon: <ClipboardCheck className="h-4 w-4 text-warning" aria-hidden />,
          title: `${a.module} awaiting approval`,
          sub: a.requester,
          to: "/approvals",
          tone: "warning",
        }),
    ...
    db.tasks
      ...
      .forEach((tk) =>
        items.push({
          icon: <ListChecks className="h-4 w-4 text-info" aria-hidden />,
          title: `Task: ${tk.title}`,
          sub: `Due ${tk.due ?? "—"} · ${tk.priority}`,
          to: "/tasks",
          tone: "info",
        }),
      );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="icon" className="relative" aria-label={t("common.notifications")}>
          ...
          {unread > 0 && (
            <span className="absolute top-2 right-2 flex h-2.5 min-w-2.5 items-center justify-center rounded-full bg-critical px-1 text-[8px] font-bold text-white" aria-hidden>
              {unread}
            </span>
          )}
        </Button>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="max-h-80 space-y-1 overflow-y-auto pr-1">
          {notifications.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">You're all caught up — no new notifications.</p>
          ) : (
            notifications.map((n, i) => (
              <button
                ...
                className="flex w-full items-start gap-3 rounded-md border border-border bg-card px-3 py-2.5 text-left transition-colors hover:bg-accent"
              >
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-muted">{n.icon}</span>
                <span className="min-w-0">
        ...
        {notifications.length > 0 && (
          <Button variant="outline" size="sm" asChild className="mt-1 w-full">
            <Link to="/approvals">View all notifications</Link>
          </Button>
        )}
      </DialogContent>
    </Dialog>
...
export function AppLayout({ children }: { children: ReactNode }) {
  const { t } = useI18n();
  const { mode, logoutLive } = useBackend();
    ...
    <div className="flex min-h-screen w-full bg-canvas">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b border-border bg-card/95 px-3 backdrop-blur sm:px-5">
          ...
          <div className="ml-auto lg:ml-0"><GlobalSearch /></div>
