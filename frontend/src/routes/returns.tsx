...
import { ArrowLeftRight, RotateCcw, RefreshCcw, CheckCheck } from "lucide-react";
        ...
        toolbarExtra={
          <ExportButton
            ...
            columns={[
              ...
              { key: "shipment", header: "Shipment", value: (r) => r.shipment },
              { key: "customer", header: "Customer", value: (r) => look.customer[r.customerId]?.name ?? "" },
              { key: "reason", header: "Reason", value: (r) => r.reason },
              { key: "condition", header: "Condition", value: (r) => r.condition },
              { key: "resolution", header: "Resolution", value: (r) => r.resolution },
        ...
        rowActions={(r) => (
          <div className="flex items-center justify-end gap-1">
            <Button
              ...
              onClick={() => {
                const next = r.status === "requested" ? "approved" : r.status === "approved" ? "received" : r.status === "received" ? "inspection" : "completed";
                update("returns", r.id, { status: next }, r.code);
                toast.success(`Return ${r.code} moved to ${next}.`);
              }}
            >
