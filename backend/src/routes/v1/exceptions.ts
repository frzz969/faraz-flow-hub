...
import { transitionFlow, EXCEPTION_FLOW, advanceShipmentStatus } from "../../lib/workflow.js";
...
exceptionsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (parsed.data.status === "resolved") {
      const rows = await import("../../lib/db.js").then((m) => m.query<{ shipment_id: number | null } & import("mysql2/promise").RowDataPacket>("SELECT shipment_id FROM exceptions WHERE id = ?", [id]));
